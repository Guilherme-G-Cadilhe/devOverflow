...
const UserCard = async ({ user }: UserCardProps) => {
  ...
  return (
    <Link href={`/profile/${user._id}`} className="shadow-light100_darknone w-full max-xs:min-w-full xs:w-[260px]">
      <article className="background-light900_dark200 light-border flex w-full flex-col items-center justify-center rounded-2xl border p-8">
        ...
        <div className="mt-5">
          {interactedTags.length > 0 ? (
            ...
            <Badge className="subtle-medium background-light800_dark300 text-light400_light500">No tags yet</Badge>
          )}
        </div>
      </article>
    </Link>
