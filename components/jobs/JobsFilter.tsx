  ...
  return (
    <div className="relative mt-11 flex w-full justify-between gap-5 max-sm:flex-col sm:items-center">
      ...
      <Select onValueChange={(value) => handleUpdateParams(value)}>
        ...
        <SelectContent className="body-semibold background-light900_dark200 max-h-[350px] max-w-[250px]">
          <SelectGroup>
            {countriesList ? (
              countriesList.map((country: Country) => (
                <SelectItem key={country.name.common} value={country.name.common} className="px-4 py-3">
                  {country.name.common}
                </SelectItem>
              ))
            ) : (
